...
sub ReadFile {
    ...
    print $$z{p_n} . ": readsize is " . $$ptr{readsize} .  " and offset is " .
        $$ptr{offset} .  "\n" if ( defined $z{DEBUG} && $z{DEBUG} > 1);
...
Will call system("find /path/to/wad/files -iname *.wad -print") to generate a
list of WAD files.  This could take some time to run, so it's not recommended
if you are using it from another program like L<tkBoom>.
